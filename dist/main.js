...
function writeResponse(nodeRes, res, body) {
    nodeRes.statusCode = res.status;
    res.headers.forEach((value, key) => nodeRes.setHeader(key, value));
    nodeRes.end(body);
}
function createChatContext(thread, message, conversationId, store) {
    ...
    const postOrUpdateMain = async (text) => {
        if (mainMessage) {
...
async function addWorkingReaction(thread, message) {
    if (thread.isDM)
        return;
    if (!message?.id)
        return;
...
async function removeWorkingReaction(thread, message) {
    if (thread.isDM)
        return;
    if (!message?.id)
        return;
...
const runThreadMessage = async (thread, message) => {
...
const gatewayLoop = async () => {
    while (!shuttingDown) {
        try {
            ...
            }, 10 * 60 * 1000, undefined, DISCORD_WEBHOOK_URL);
            if (!response.ok) {
