...
A Discord bot powered by an LLM that can execute bash commands, read/write files, and interact with your development environment. Mom is **self-managing**. She installs her own tools, programs [CLI tools (aka "skills")](https://mariozechner.at/posts/2025-11-02-what-if-you-dont-need-mcp/) she can use to help with your workflows and tasks, configures credentials, and maintains her workspace autonomously.
...
# Optional auth for Anthropic (if not using ~/.pi/mom/auth.json)
...
Mom is a Node.js app that runs on your host machine. She connects to Discord via Chat SDK (webhook + gateway listener), receives messages, and responds using an LLM-based agent that can create and use tools.
